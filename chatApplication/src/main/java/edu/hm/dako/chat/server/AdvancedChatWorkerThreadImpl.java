...
public class AdvancedChatWorkerThreadImpl extends AbstractWorkerThread {
		...
		for (String s : new Vector<String>(clientList2)) {
			...
			try {
				if (client != null) {
					...
					// Zähler für die eingeloggten und ausgeloggten User
					log.debug(userName + ": Anzahl der Login/Logout-Events = " + eventCounter.get()
							+ ", Anzahl der Login/Logout-Event-Confirms =" + confirmCounter.get());
	...
	protected void loginRequestAction(ChatPDU receivedPdu) {
		...
		if (!clients.existsClient(receivedPdu.getUserName())) {
			...
			// Erstellung der ClientWaitList um Confirm abzuwarten
			// NECIBEDA
	...
	protected void handleIncomingMessage() throws Exception {
		...
		try {
			...
			case CONFIRM_LOGOUT_EVENT:
				// Client bestaetigt Logout
				confirmLogoutEventAction(receivedPdu);
				break;

			case CONFIRM_EVENT:
				// Client bestaetigt Empfang einer Nachricht
	...
	/**
	 * Bearbeitung des Login-Confirms-Events
	 * 
	 * @author NECIBEDA
	 * @param receivedPdu
	 * @throws Exception
	 */
	private void confirmLoginEventAction(ChatPDU receivedPdu) throws Exception {

		// Zähler für die Anzahl der Event Confirms
		clients.incrNumberOfReceivedChatEventConfirms(receivedPdu.getEventUserName());
		confirmCounter.getAndIncrement();

		// Der aktuelle Client der das Confirm gesendet hat wird aus der
		// Login-Event-Wait-List gelöscht
		// NECIBEDA
		clients.deleteWaitListEntry(receivedPdu.getEventUserName(), receivedPdu.getUserName());

		// Prüfung ob die Waitlist leer ist
		// NECIBEDA
		if (clients.getWaitListSize(receivedPdu.getEventUserName()) == 0) {

			// Das Response wird erst nach dem Empfang aller Login-Confirm-Events gesendet
			// Teil von der Methode (loginRequestAction) kopiert und erweitert
			// NECIBEDA
	...
	/**
	 * Bearbeitung des Logout-Confirms-Events
	 * 
	 * @author NECIBEDA
	 * @param receivedPdu
	 * @throws Exception
	 */
	private void confirmLogoutEventAction(ChatPDU receivedPdu) throws Exception {
		...
		if (clients.getWaitListSize(receivedPdu.getEventUserName()) == 0) {
			...
			// Das Response wird erst nach dem Empfang aller Logout-Confirm-Events gesendet
			// Teil von der Methode (loginRequestAction) kopiert
			// NECIBEDA
	...
	/**
	 * Bearbeitung des Message-Confirms-Events
	 * 
	 * @author NECIBEDA
	 * @param receivedPdu
	 * @throws Exception 
	...
	private void confirmEventAction(ChatPDU receivedPdu) throws Exception {
		...
		// Der aktuelle Client der das Confirm gesendet hat wird aus der
		// Message-Event-Wait-List gelöscht
		// NECIBEDA
		clients.deleteWaitListEntry(receivedPdu.getEventUserName(), receivedPdu.getUserName());
		...
		if (clients.getWaitListSize(receivedPdu.getEventUserName()) == 0) {
			...
			if (!clients.existsClient(receivedPdu.getEventUserName())) {
				log.debug("User nicht in Clientliste: " + receivedPdu.getEventUserName());
